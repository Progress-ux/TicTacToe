...
Для игры по сети у игроков должен быть прямой доступ к IP-адресу друг друга. Я знаю два основных способа настроить соединение:

### 1. Локальная сеть (LAN)

Если вы находитесь в одной Wi-Fi сети, просто узнайте свой локальный IP (команда `ipconfig` для Windows или `ifconfig/ip` a для Linux) и введите его в `config.json` клиента.

### 2. Виртуальные сети (VPN)

Если вы играете через интернет, проще всего использовать эмуляторы локальной сети. Они создают "виртуальный кабель" между вашими компьютерами.

- **Radmin VPN / Hamachi:** Создайте комнату, попросите друга подключиться к ней и используйте IP-адрес, который выдаст программа.
  - Примечание: я тестировал игру через Radmin VPN, это работает стабильно.
